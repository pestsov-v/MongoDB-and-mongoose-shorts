...
| $slice | Возвращает подмножество документов | Возвращает массив документов в ограниченном количестве. <br/> Если нужны документы с конца необходимо поставить "-" перед числом. <br/> Если нужны документы из средины массива документов - первым аргументов обозначается элемент с которого начать, вторым аругментом - количество документов | Post.find({"comments": {$slice: 3}}) <br/> Post.find({"comments": {$slice: -3}}) <br/> Post.find({"comments": {$slice: [3, 10]}}) |
