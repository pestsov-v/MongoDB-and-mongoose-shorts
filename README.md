...
- [Запросы для определённых типов данных](#запросы-для-определённых-типов-данных)
    - [Операторы по фильтрации массива документов](#операторы-по-фильтрации-массива-документов)
    - [Запросы для типов Null и регулярных выражений](#запросы-для-типов-null-и-регулярных-выражений)
- [Геопространственные запросы](#геопространственные-запросы)
    - [Пример $near](#пример-$near)
...
- [Лимитирование, пропуск данных, сравнение и сортировка](#лимитирование-пропуск-данных-сравнение-и-сортировка)
    - [Методы обработки документов](#методы-обработки-документов)
    - [Порядок сравнения](#порядок-сравнения)
- [Аккумуляторы](#аккумуляторы)
- [Агрегации](#агрегации)
    - [Механизм агрегации](#механизм-агрегации)
    - [Операторы агрегации](#операторы-агрегации)
        - [Детально про $geoNear](#детально-про-$geonear)
            - [Опции оператора $geoNear](#опции-оператора-geonear)
        - [Детально про $project](#детально-про-$project)
        - [Детально про $unwind](#детально-про-$unwind)
        - [Детально про $push](#детально-про-$push)
    - [Применение выражений в фреймворке агрегаций](#применение-выражений-в-фрейморке-агрегаций)
- [Индексы](#индексы)
    - [Неефективное использование](#неефективное-использование)
    - [Неефективные индексы](#неефективные-индексы)
    - [Когда применять индексы](#когда-применять-индексы)
    - [Геопространственные индексы](#геопространственные-индексы)
        - [Точка в 2dshpere](#точка-в-2dshpere)
        - [Линия в 2dshpere](#линия-в-2dshpere)
        - [Полигон в 2dshpere](#полигон-в-2dshpere)
- [Нормализация и денормализация](#нормализация-и-денормализация)
    - [$lookup - этап агрегации](#$lookup-этап-агрегации)
- [Model](#model)
    - [Опции](#опции)
        - [Опции описывающие все типы](#опции-описывающие-все-типы)
        - [Опции описывающие индексы](#опции-описывающие-индексы)
        - [Опции описывающие строку](#опции-описывающие-строку)
        - [Опции описывающие числа](#опции-описывающие-числа)
        - [Опции описывающие дату](#опции-описывающие-дату)
        - [Форма записи опций](#форма-записи-опций)
- [Populate](#populate)
    - [Получение определённых полей для заполнения документов](#получение-определённых-полей-для-заполнения-документов)
    - [Populate нескольких коллекций](#populate-нескольких-коллекций)
    - [Запрос с условием](#запрос-с-условием)
    - [Populate в промежуточных обработчиках](#populate-в-промежуточных-обработчиках)
